...
■ 今後1週間の注目ライブ・イベント（{upcoming_count}件、大型会場開催またはメンバー動向系のみ）
...
{{
  "scene_summary": "ライブアイドルシーン全体の動向を300〜450文字でまとめたナレーション（必須）",
  "spotlight_topics": [
    {{
  ...
  "sns_buzz_topics": [
    {{
      ...
      "description": "SNSやメディアで注目されている理由と概要（80〜150文字）",
      "reason": "なぜ話題か（30文字以内）"
    }}
  ],
  "upcoming_events": "今後の注目ライブ・イベント情報（150〜300文字・「今後1週間の注目ライブ・イベント」のデータを優先して使用し、複数件あれば列挙する）",
...
class AIAnalyzer:
    ...
    def _fallback_analysis(self, target_date: str, news: list[dict]) -> dict:
        ...
        for n in sns_trending[:3]:
            ...
            description = title[:100] if title else summary[:100]
